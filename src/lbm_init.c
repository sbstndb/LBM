...
/*******************  FUNCTION  *********************/
/**
 * Initialisation de l'obstacle, on bascule les types des mailles associés à
 *CELL_BOUNCE_BACK. Ici l'obstacle est un cercle de centre
 *(OBSTACLE_X,OBSTACLE_Y) et de rayon OBSTACLE_R.
 **/
    ...
    for (j = mesh_comm->y; j < mesh->height + mesh_comm->y; j++) {
      if (((i - OBSTACLE_X) * (i - OBSTACLE_X)) +
        ...
        *(lbm_cell_type_t_get_cell(mesh_type, i - mesh_comm->x,
                                   j - mesh_comm->y)) = CELL_BOUNCE_BACK;
...
/*******************  FUNCTION  *********************/
/**
 * Initialise le fluide complet avec un distribution de poiseuille correspondant
 *un état d'écoulement linéaire à l'équilibre.
  ...
  for (i = 0; i < mesh->width; i++) {
    for (j = 0; j < mesh->height; j++) {
      for (k = 0; k < DIRECTIONS; k++) {
        ...
        Mesh_get_cell(mesh, i, j)[k] =
            compute_equilibrium_profile(v, density, k);
        // mark as standard fluid
        *(lbm_cell_type_t_get_cell(mesh_type, i, j)) = CELL_FUILD;
        // this is a try to init the fluide with null speed except on left
        // interface. if (i > 1) 	Mesh_get_cell(mesh, i, j)[k] = equil_weight[k];
      }
    }
  }
}
 ...
 * @param mesh Le maillage à initialiser.
 * @param mesh_type La grille d'information notifiant le type des mailles.
 * @param mesh_comm La structure de communication pour connaitre notre position
 *absolue dans le maillage globale.
 **/

...
bool read_next_frame(lbm_data_file_t *file) {
  ...
  // load the frame
  res = fread(file->entries, sizeof(lbm_file_entry_t),
...
bool seek_to_frame(lbm_data_file_t *file, int frame) {
  int res = fseek(file->fp,
                  frame * sizeof(lbm_file_entry_t) * file->header.mesh_height *
  ...
  for (i = 0; i < file->header.mesh_width; i++) {
    for (l = 0; l < file->header.lines; l++) {
      for (j = 0; j < line_height; j++) {
        ...
        printf("%d %d %f %f\n", i, j + l * line_height,
               file->entries[pos].density, file->entries[pos].v);
      }
    }
    printf("\n");
  }
...
int get_frame_count(lbm_data_file_t *file) {
  ...
  if (fstat(fileno(file->fp), &info) == 0)
    return info.st_size / (file->header.mesh_width * file->header.mesh_height *
                           sizeof(lbm_file_entry_t));
  else
    return 0;
}

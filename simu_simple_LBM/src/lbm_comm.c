...
void lbm_comm_print(lbm_comm_t *mesh_comm) {
  ...
  printf(" RANK %d ( LEFT %d RIGHT %d TOP %d BOTTOM %d CORNER %d, %d, %d, %d ) "
         "( POSITION %d %d ) (WH %d %d ) \n",
         rank, mesh_comm->left_id, mesh_comm->right_id, mesh_comm->top_id,
         mesh_comm->bottom_id, mesh_comm->corner_id[0], mesh_comm->corner_id[1],
         mesh_comm->corner_id[2], mesh_comm->corner_id[3], mesh_comm->x,
 ...
 **/
void lbm_comm_init(lbm_comm_t *mesh_comm, int rank, int comm_size, int width,
                   int height) {
  // vars
  int nb_x;
  int nb_y;
  ...
  if (nb_y > 1) {
    ...
    //    mesh_comm->buffer_bottom = malloc(sizeof(double) * DIRECTIONS * width
    //    / nb_x)
  } else {
    mesh_comm->buffer = NULL;
    //    mesh_comm->buffer_bottom = NULL;
  }
...
#if NAIVEMPI
    // printf("hello from horizontal - send to %d\n", target_rank);
    for (int y = 0; y < mesh->height - 2; y++) {
      MPI_Send(&Mesh_get_col(mesh_to_process, x)[y * DIRECTIONS], DIRECTIONS,
               MPI_DOUBLE, target_rank, y, MPI_COMM_WORLD);
    }
#elif FACTORIZEDMPI
    MPI_Send(&Mesh_get_col(mesh_to_process, x)[0],
             DIRECTIONS * (mesh->height - 2), MPI_DOUBLE, target_rank, 0,
             MPI_COMM_WORLD);
#endif
...
#if NAIVEMPI
    // printf("hello from horizontal - recv from %d\n", target_rank);
    for (int y = 0; y < mesh->height - 2; y++) {
      MPI_Recv(&Mesh_get_col(mesh_to_process, x)[y * DIRECTIONS], DIRECTIONS,
               MPI_DOUBLE, target_rank, y, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
#elif FACTORIZEDMPI
    MPI_Recv(&Mesh_get_col(mesh_to_process, x)[0],
             DIRECTIONS * (mesh->height - 2), MPI_DOUBLE, target_rank, 0,
 ...
 * Debut de communications asynchrones
 * @param mesh_comm MeshComm à utiliser
 * @param mesh_to_process Mesh a utiliser lors de l'échange des mailles fantome
 **/
void lbm_comm_sync_ghosts_diagonal(lbm_comm_t *mesh, Mesh *mesh_to_process,
  ...
  switch (comm_type) {
  case COMM_SEND:
    MPI_Send(Mesh_get_cell(mesh_to_process, x, y), DIRECTIONS, MPI_DOUBLE,
             target_rank, 0, MPI_COMM_WORLD);
    break;
  case COMM_RECV:
    MPI_Recv(Mesh_get_cell(mesh_to_process, x, y), DIRECTIONS, MPI_DOUBLE,
...
void lbm_comm_sync_ghosts_vertical(lbm_comm_t *mesh, Mesh *mesh_to_process,
  ...
  case COMM_RECV:
    ...
    for (x = 1; x < mesh_to_process->width - 2; x++) {
      for (k = 0; k < DIRECTIONS; k++) {
        MPI_Recv(&Mesh_get_cell(mesh_to_process, x, y)[k], DIRECTIONS,
...
void lbm_comm_ghost_exchange(lbm_comm_t *mesh, Mesh *mesh_to_process) {
  ...
  lbm_comm_sync_ghosts_vertical(mesh, mesh_to_process, COMM_RECV, mesh->top_id,
                                0);
  ...
  lbm_comm_sync_ghosts_vertical(mesh, mesh_to_process, COMM_SEND, mesh->top_id,
                                1);
...
#if BARRIER
  // printf("je suis dans la barriere\n");
  //  prevend comm mixing to avoid bugs
  MPI_Barrier(MPI_COMM_WORLD);
#endif
